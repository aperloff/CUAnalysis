...
FormattedPlot::FormattedPlot(TH1* templateHisto_, std::vector<std::string> axisTitles_, std::pair<double,double> range_) :
Plot(templateHisto_),
axisTitles(axisTitles_),
range(range_)
{
   overlaySignalFactor = 1.0;
   overlaySignalName = "";
...
vector<TCanvas*> FormattedPlot::getCanvas1DTDR(std::vector<PhysicsProcess*> procs) {
    ...
    // Create the comparison stacks
    TString tempName = templateHisto->GetName();
    THStack * stack = new THStack(tempName+"_stack",tempName+"_stack");

    // Make the legend
    TLegend * l = st->tdrLeg(0.44,0.64,0.88,0.89);
    l->SetName(tempName+"_legend");
    l->SetHeader("Process                    (Entries)");
    l->SetNColumns(2);
    ...
    for (unsigned int h = 0 ; h < groupedHistos.size() ; h++) {
        ...
        l->AddEntry((TObject*)0,Form("(%.0f)",groupedHistos[h]->Integral()),"");
        stack->Add(groupedHistos[h],"hist");
        maximums.push_back(groupedHistos[h]->GetBinContent(groupedHistos[h]->GetMaximumBin()));
    }
    ...
    for (unsigned int igroup=0; igroup<groupedHistos.size(); igroup+=2) {
        TH1* hRatio = (TH1*) groupedHistos.at(igroup+1)->Clone(tempName+"_Ratio");
        hRatio->SetTitle("HEM/Nominal");
        hRatio->Divide(groupedHistos.at(igroup));
        st->tdrDraw(hRatio,"",kFullCircle,groupedHistos.at(igroup)->GetMarkerColor(),kSolid,groupedHistos.at(igroup)->GetLineColor());
    }
...
// ------------------------------------------------------------
void FormattedPlot::formatColorsStack(vector<PhysicsProcess*> procs)
{
   for(unsigned int i = 0; i < histos.size(); i++) {
      int proc_index = 0;
      for(unsigned int p = 0; p < procs.size(); p++) {
